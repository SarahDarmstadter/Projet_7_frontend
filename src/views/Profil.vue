<template>
            ...
            <!-- Navbar links -->
                ...
                <div class="collapse navbar-collapse" id="collapsibleNavbar">
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item">
                            <button class="nav-link btn btn-outline-danger"><a class="nav-link"  href="#">Forum</a></button>
                        </li>
                    </ul>
                </div>
                </nav>
                <div class="dropdown" id="dropdown">
                    ...
                    <a class="dropdown-link" href="#">Forum</a>
                </div>
        </header>
    <div class="container-fluid">
        ...
        <button class="suppression btn-profil" @click="deleteAccount()">Supprimer mon compte</button>
    </div>
    </div>   
    </div>
...
//modifier le userName 
        changeUsername : function(){
            let divUnsername = document.getElementById("unvisible");
            divUnsername.classList.toggle("unvisible")
        },
        ...
        newUsername : function() {
            ...
            if(this.$store.state.token !==""){
                try {  
                    axios.put('http://localhost:3000/api/auth/profil/update', 
                        {userName : this.userName}, 
                        {headers : {"Authorization" : `Bearer ${this.$store.state.token}`}
                    })
                        .then(function(response) {
                            console.log("response axios.put username", response)
                            self.$store.dispatch('getUserProfil', self.userName)
                            console.log("SelfuserName : nouveau name", self.userName)
                        })
                        .catch(function(error){
                        console.log(error)
                        })
                } catch(error) {
                    console.log("error du catch axios.put", error)
                    } 
...
//modifier l'image du profil 
        ...
        selectFile(event) {
            ...
            this.newImage = URL.createObjectURL(this.newFile)  
        },
...
// suppression du compte 
       deleteAccount : function() {
            ...
            .then(function(){
                //self.$router.push({ path:"/", name:"Home"})
            })
            .catch(function(err) {
                console.log(err)
            })
